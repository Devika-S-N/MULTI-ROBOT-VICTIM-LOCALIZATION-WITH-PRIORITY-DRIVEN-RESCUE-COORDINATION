            ...
            for vi, v in enumerate(victims):
                if np.linalg.norm(pos - v.pos) <= robot_cfg.sensor_range:
                    ...
                    if not r.logged_victim_detection[vi]:
                        ...
                        if not hasattr(r, "detection_time_phase2"):
                            r.detection_time_phase2 = {}

                        if vi not in r.detection_time_phase2:   # store earliest
                            r.detection_time_phase2[vi] = k
