...
def run_phase1(
    ...
    for k in range(phase_cfg.n_steps):
        ...
        for r in robots:
            ...
            # log beliefs for all victims
            r.belief_history_phase1.append(r.belief_victims.copy())
            # Compute belief variance across all victims (per robot)
            belief_var = np.var(r.belief_victims, axis=0)
            r.belief_var_history_phase1.append(belief_var)
        ...
        for r in robots:
            ...
            if victims_in_range:
                for vi in victims_in_range:
                    ...
                    # --- (robot-level detection logging) ---
                    if vi not in r.detection_time_phase1:
                        r.detection_time_phase1[vi] = k
                    ...
                    # --- store noisy measurement for later plotting ---
                    r.meas_noise_scatter_phase1.append((z.copy(), v.pos.copy()))
            ...
            # --- Case B: no victim, but false positives nearby ---
                ...
                if rng.random() < phase_cfg.fp_confusion_prob:
                    ...
                    # --- log FP confusion & noisy measurement ---
                    r.fp_confusion_events_phase1.append(k)
                    r.meas_noise_scatter_phase1.append((z_fp.copy(), fp_pos.copy()))
